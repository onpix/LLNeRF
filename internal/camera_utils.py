...
def cast_ray_batch(
  ...
  Args:
    ...
    camtype: camera_utils.ProjectionType, fisheye or perspective camera.
    xnp: either numpy or jax.numpy.
    phase: 'train' for doing in the training step or 'test' for doing in the validation and test dataset.
  ...
  # In training, shape of tensors are: [2048, 1, ...];
  # In test and validation, are [image_h, image_w, ...]
  cam_idx = pixels.cam_idx[..., 0]
  batch_index = lambda arr: arr if arr.ndim == 2 else arr[cam_idx]

  # Compute rays from pixel coordinates.
  # train: shape [2048, 1, 1]; test: shape [img_h, img_w]
  # origins, directions, viewdirs, radii, imageplane = pixels_to_rays(pixels.pix_x_int,pixels.pix_y_int,batch_index(pixtocams),batch_index(camtoworlds),distortion_params=distortion_params,pixtocam_ndc=pixtocam_ndc,camtype=camtype,xnp=xnp)
  if phase == 'train' and sample_neighbor_num > 0:
    # right_pix = (pixels.pix_x_int + 1, pixels.pix_y_int)
    # left_pix = (pixels.pix_x_int - 1, pixels.pix_y_int)
    # up_pix = (pixels.pix_x_int, pixels.pix_y_int - 1)
    # down_pix = (pixels.pix_x_int, pixels.pix_y_int + 1)
    all_pixels_x = jnp.concatenate(
      (
        pixels.pix_x_int,
        pixels.pix_x_int + 1,
        # pixels.pix_x_int - 1,
    ...
    all_pixels_x = pixels.pix_x_int
    all_pixels_y = pixels.pix_y_int
