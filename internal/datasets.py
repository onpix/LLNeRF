  ...
  def _make_ray_batch(self,
    ...
    if self._cast_rays_in_train_step and self.split == utils.DataSplit.TRAIN:
      ...
      rays = camera_utils.cast_ray_batch(
          self.cameras, pixels, (self.width, self.height), self.camtype, xnp=np, phase='test')
...
class LLFF(Dataset):
  ...
  def _load_renderings(self, config):
    ...
    if config.rawnerf_mode:
      ...
      img_sizes = set([x.shape for x in images])
      if len(img_sizes) > 1:
        print(f'Find different size: {img_sizes} in training images.')
