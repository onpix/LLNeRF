...
def visualize_suite(rendering, rays):
  ...
  try:
    ...
    print(f'Error: {e} when getting vis_depth_mean and vis_depth_median')
    vis_depth_mean = vis_depth_median = None
  ...
  try:
    ...
    print(f'Error: {e} when getting vis_depth_triplet')
    vis_depth_triplet = None
  ...
  try:
    ...
    print(f'Error: {e} when getting vis_ray_colors and vis_ray_weights')
    vis_ray_colors = vis_ray_weights = None

  try:
    color_matte = matte(rgb, acc)
  except Exception as e:
    print(f'Error: {e} when getting color_matte')
    color_matte = None

  try:
    coords_mod = visualize_coord_mod(coords, acc)
  except Exception as e:
    print(f'Error: {e} when getting v')
    coords_mod = None

  # vis L, R, rgb_enhanced
  extra_keys = [
    'L', 'R', 'R_norm',
    'Lx8', 'rgb_Lx8',
  ...
  extra_vis = {k: rendering[k] for k in extra_keys if k in rendering and rendering[k] is not None}
  if 'R' in extra_vis:
    extra_vis['R_norm'] = extra_vis['R'] / extra_vis['R'].max()

  vis = {
    'color': rgb,
    'acc': acc,
    'color_matte': color_matte,
    'depth_mean': vis_depth_mean,
    'depth_median': vis_depth_median,
    'depth_triplet': vis_depth_triplet,
    'coords_mod': coords_mod,
