...
def main(unused_argv):
  ...
  while True:
    ...
    for idx in range(dataset.size):
      ...
      filename = path.basename(dataset.image_paths[idx])
      batch = next(dataset)
      if idx >= num_eval:
      ...
      if config.eval_save_output and (config.eval_render_interval > 0):
        if (idx % config.eval_render_interval) == 0:
          utils.save_img_u8(postprocess_fn(rendering['rgb']),
                            path_fn(f'{filename}_color_{idx:03d}.png'))
          utils.save_img_u8(postprocess_fn(rendering['rgb_cc']),
                            path_fn(f'{filename}_color_cc_{idx:03d}.png'))

          if 'L' in rendering:
            # save R_norm
            utils.save_img_u8(rendering['R'] / rendering['R'].max(), path_fn(f'{filename}_R_norm_{idx:03d}.png'))
            utils.save_img_u8(rendering['L'] / rendering['L'].max(), path_fn(f'{filename}_L_norm_{idx:03d}.png'))
            ...
            for render_name in rendering.keys():
              if render_name in EXTRA_VIS_KEYS:
                ...
                utils.save_img_u8(render_tensor, path_fn(f'{filename}_{render_name}_{idx:03d}.png'))

          for key in ['distance_mean', 'distance_median']:
            if key in rendering:
              utils.save_img_f32(rendering[key],
                                 path_fn(f'{filename}_{key}_{idx:03d}.tiff'))
